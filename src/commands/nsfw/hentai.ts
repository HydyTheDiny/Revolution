...
export default new Command("hentai")
  .setPermissions("bot", "embedLinks")
  .setDescription('Searches the internet for hentai nsfw images')
  .setUsage('hentai')
  .setExecutor(async function(msg) {
    if(msg.channel.nsfw === true){
        let data = await req("https://nekobot.xyz/api/image")
        .query("type", "hentai" ).json<{success: boolean, message: string, color: number, version: string}>()
