...
export default new Command("hass")
  .setPermissions("bot", "embedLinks")
  .setDescription('Searches the internet for hentai ass nsfw images')
  .setUsage('hass')
  .setExecutor(async function(msg) {
    if(msg.channel.nsfw === true){
        let data = await req("https://nekobot.xyz/api/image")
        .query("type", "hass" ).json<{success: boolean, message: string, color: number, version: string}>()
