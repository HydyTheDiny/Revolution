...
export default new Command("holo")
  .setPermissions("bot", "embedLinks")
  .setDescription('Searches the internet for holo nsfw images')
  .setUsage('holo')
  .setExecutor(async function(msg) {
    if(msg.channel.nsfw === true){
        let data = await req("https://nekobot.xyz/api/image")
        .query("type", "holo" ).json<{success: boolean, message: string, color: number, version: string}>()
