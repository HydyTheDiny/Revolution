...
export default new Command("hthigh")
  .setPermissions("bot", "embedLinks")
  .setDescription('Searches the internet for hentai thighs nsfw images')
  .setUsage('hthigh')
  .setExecutor(async function(msg) {
    if(msg.channel.nsfw === true){
        let data = await req("https://nekobot.xyz/api/image")
        .query("type", "hthigh" ).json<{success: boolean, message: string, color: number, version: string}>()
